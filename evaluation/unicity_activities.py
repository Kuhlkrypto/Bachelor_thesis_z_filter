...
def risk_re_ident_quant(path_data_sources, csv_source_file_name, projection='A'):
    ...
    # Specify columns
    unique_identifier = [cons.COL_NAME_CASE_IDENT]
    case_attribute = []
    activity = [cons.COL_NAME_ACTIVITY]
    event_attribute = []
    timestamp = [cons.COL_NAME_TIMESTAMP]
    ...
    values_relative = [math.ceil(i * number_points_total) for i in sorted(cons.RISK_POINTS_RELATIVE)]
    # # # # # # # # # # # # # # # # # # # # #
    ...
    if cons.RISK_MULTIPROCESSING:
        with ProcessPoolExecutor() as executor:
            futures_rel = []
            for val in values_relative:
                futures_rel.append(
                    executor.submit(calculate_unicity, df_aggregated_data, quasi_identifier, events_to_concat, val))
            futures_abs = []
            for val in cons.RISK_POINTS_ABSOLUTE:
                futures_abs.append(
                    executor.submit(calculate_unicity, df_aggregated_data, quasi_identifier, events_to_concat, val))

            for future in futures_rel:
                res_rel.append(future.result())
            for future in futures_abs:
                res_abs.append(future.result())

    return sorted(res_rel, key=lambda x: x[0]), sorted(res_abs, key=lambda x: x[0])
